...
impl Decimal {
    ...
    fn try_push_digit(&mut self, digit: u8) {
    ...
    pub fn new<I: Input>(input: &I) -> Decimal {
        let mut d = Decimal::default();
        let mut digits = input.integer_digits();
        while let Some(digit) = digits.next() {
            let digit = digit.value();
            if digit != 0 {
                d.try_push_digit(digit);
                for digit in digits {
                    d.try_push_digit(digit.value());
                }
                break;
            }
        }
        if d.num_digits == 0 {
            ...
            while let Some(digit) = digits.next() {
                ...
                if digit != 0 {
                    d.try_push_digit(digit);
                    for digit in input.fraction_digits() {
                        d.try_push_digit(digit.value());
                        d.decimal_point -= 1;
                    }
                    break;
        ...
        if d.num_digits != 0 {
            'trailing: loop {
                for digit in input.fraction_digits().rev() {
                    if digit.value() == 0 {
                        d.try_pop_dight();
                        d.decimal_point += 1;
                    } else {
                ...
                for digit in input.integer_digits().rev() {
                    if digit.value() == 0 {
                        d.try_pop_dight();
                        d.decimal_point += 1;
                    } else {
            ...
            if d.num_digits > Self::MAX_DIGITS {
                d.truncated = true;
                d.num_digits = Self::MAX_DIGITS;
            }
        }
        d.decimal_point += input.exponent() as i32;
        d
    }
}
