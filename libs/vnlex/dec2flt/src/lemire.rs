use crate::{raw_float::RawFloat, table::{SMALLEST_POWER_OF_FIVE, LARGEST_POWER_OF_FIVE, POWER_OF_FIVE_128}};
...
impl BiasedFp {
    ...
    pub fn inf<F: RawFloat>() -> Self {
        Self { f: 0, e: F::INFINITE_POWER }
    }

    pub fn compute<F: RawFloat>(q: i64, mut w: u64) -> Option<BiasedFp> {
        if w == 0 || q < F::SMALLEST_POWER_OF_TEN as i64 {
            return Some(Default::default());
        }
        if q > F::LARGEST_POWER_OF_TEN as i64 {
            return Some(BiasedFp { f: 0, e: F::INFINITE_POWER });
        }
        ...
        if power2 <= 0 {
            ...
            return Some(BiasedFp { f: mantissa, e: power2 });
        }
        ...
        Some(BiasedFp { f: mantissa, e: power2 })
    }
